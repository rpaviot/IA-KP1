...
class survey_geometry:
    ...
    def __init__(self,*args,**qwargs):
        ...
        cond = np.where(self.ra < 0)[0]
        if ((len(cond) > 0)):    
            ra2 = self.convert_ra(self.ra)
            self.ra_conversion = True
        else:
            ra2 = self.ra
    ...
    def set_target_nz(self,**qwargs):
        ...
        self.dn_data_ = np.copy(self.dn_data)
    ...
    def compute_nz(self):
        ...
        dr = np.diff(edges_r)
        self.n_z = self.dn_data_/(4*np.pi*dist**2*dr*(self.area/self.sphere_area))
        return self.centerbin,self.edges,self.dn_data_,self.n_z
    ...
    def create_random(self,size_random,shuf=False):
        ...
        dec_rand = rad2deg(np.arccos(sindec_rand)-np.pi/2.)
        ra_rand,dec_rand = self.infootprint(ra_rand,dec_rand)
        
        if self.ra_conversion == True:
    ...
    def create_random_angular_easy(self,size_random):
        if self.ra_conversion == True:
            ...
            self.ra_rand = rng.uniform(min(self.ra), max(self.ra), size_random)
            sindec_rand = rng.uniform(np.sin(min(self.dec*np.pi/180)), np.sin(max(self.dec*np.pi/180)), size_random)
            self.dec_rand = rad2deg(np.arcsin(sindec_rand))           
    
    """Work only for simplest rectangular footprint without masks"""
    def create_random_easy(self,size_random,shuf=False,random=False):
        if self.ra_rand is None:
            self.create_random_angular_easy(size_random)    
        if random is True:
            z_rand = rng.uniform(self.xmin,self.xmax,size_random)
        elif shuf is True:
